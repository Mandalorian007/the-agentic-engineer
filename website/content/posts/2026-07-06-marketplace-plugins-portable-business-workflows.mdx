...
social:
  twitter:
    text: "🔌 Stop wiring business AI to one vendor.\n\nMarketplace plugins ship 5 portable primitives that move across Claude Code, Codex, Gemini, and Cursor.\n\nThe full coverage matrix 👇"
  linkedin:
    text: "Every AI vendor is in a race. They raced on knowledge, then on reasoning, now on agentics. Whatever comes next, they'll race on that too.\n\nMost companies are pouring AI investment directly into picking the racer. That bet doesn't survive contact with the next quarter.\n\nThere's a better layer to bet on. Marketplace plugins are emerging as an open spec, and they ship the primitives every major agent platform supports or is racing to support.\n\nFive primitives, all portable across vendors:\n\n1. Slash commands. Named entry points a user invokes directly. Good for packaging a workflow as a single button.\n\n2. Skills. Auto-invocable capability blocks the agent pulls in when relevant. The agent decides when to apply a skill based on context.\n\n3. Sub-agents. Spawned helpers the parent agent delegates to. Useful for parallel research, isolated tool permissions, or specialized roles.\n\n4. Hooks. Interception points around lifecycle events. Pre-tool-use checks, post-edit validation, session-start setup. The plumbing that enforces safety.\n\n5. MCPs. Not strictly plugin-native, but plugins bundle MCP servers, and the MCP layer is where your actual tool surface lives.\n\nThat's the harness. Commands and skills are how work enters. Sub-agents are how it parallelizes. Hooks are how it stays safe. MCPs are how it touches your business.\n\nCoverage across the four agent platforms that matter is already strong. Claude Code is full across the board. Gemini CLI is the closest match for portability. Codex CLI ports commands and skills cleanly but is weak on sub-agents and hooks. Cursor is strong on sub-agents and hooks but converts skills into Rules with different activation behavior.\n\nNone of those gaps are blockers. They're gotchas to track as you port.\n\nThe honest rule of thumb: design at the primitive level, port at the platform level. Your harness is portable. Some shimming is the cost of admission.\n\nIf you have an MCP collection but no workflows on top of it, this is the layer to invest in next.\n\nFull breakdown and the coverage matrix:"
---
