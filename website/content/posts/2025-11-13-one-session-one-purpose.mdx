...
social:
  twitter:
    text: "⚡ Long AI coding sessions = degraded performance + catastrophic mistakes\n\nSolution: One session, one purpose\n\n/clear between phases reclaims 22.5% context 👇"
  linkedin:
    text: "You've seen the horror stories: \"Claude wiped my entire Git repo.\" \"It force-pushed to main and destroyed the project.\"\n\nIf you've worked with AI coding agents, you know that feeling when sessions get... funky. Performance degrades. Decisions get strange. Mistakes multiply.\n\nThe problem isn't the AI. It's how we're managing context.\n\nLong sessions accumulate noise:\n• More mistakes (agent confused by earlier context)\n• Less accuracy (precision drops as context fills)\n• Strange behavior (increasingly unpredictable)\n• Catastrophic actions (in worst cases)\n\nWorse: Auto-compact reserves 22.5% of your 200K context window (45,000 tokens!) for compression.\n\nThe solution? One session, one purpose.\n\nInstead of one massive session:\nPlan → implement → test → review → document\n\nBreak into focused sessions:\n• /prime → /plan → save spec.md → /clear\n• /prime → /implement spec.md → /clear\n• /prime → /test spec.md → /clear\n• /prime → /review spec.md → /clear\n\nEach session starts fresh. State lives in files. Context stays clean.\n\nBenefits:\n• Reclaim 22.5% context (disable auto-compact)\n• Prevent catastrophic mistakes (isolated scope)\n• Enable parallel development (Git worktrees)\n• Better results (targeted context)\n\nThis isn't just about avoiding disasters. It's about building reliable engineering pipelines powered by AI.\n\nComplete one-session-one-purpose guide:"
---
...
If you've worked with AI coding agents for any length of time, you know the feeling. A session starts to get a little funky. The agent's thinking becomes less accurate. It makes strange decisions. Performance degrades.

**The problem isn't the AI. It's how we're managing context.**

This post walks through the exact pattern I use to prevent these disasters while unlocking capabilities most developers never attempt. It's called **one session, one purpose**, and it's the difference between babysitting an unpredictable assistant and running a reliable engineering machine.

## The hidden problem: context windows are working against you

What's actually happening inside your AI coding sessions?

### Long sessions degrade performance

The longer your session runs, the more information your agent has to consider with every action. That sounds obvious, but the implications are serious:
...
You're asking the agent to hold an ever-growing pile of information in working memory while writing precise code. It doesn't scale.

### Auto-compact: the 22.5% tax on your context

Most developers don't know this. When you're using Claude Code, **22.5% of your 200K token context window is reserved for auto-compact**.

Run `/context` in Claude Code right now and check. That's roughly **45,000 tokens**, nearly a quarter of your available space, set aside just to handle context compression when your window gets too full.
...
Here's the kicker: **you probably don't need auto-compact at all** if you structure your workflow correctly.
...
Instead of one massive session that tries to plan, implement, test, review, and document all at once, break it into focused sessions:
...
Here's exactly how this works in practice. I'll use Claude Code slash commands as examples, but the pattern works with any AI coding tool.
...
After this, the agent has a high-level understanding of your application without being buried in implementation details.
...
This generates a detailed spec file outlining what needs to be built.

**Key insight:** All the research, web searches, and exploratory work happens here. The agent might investigate 12 different approaches, read documentation, compare libraries. That creates a ton of context.
...
Your agent implements the feature with:
- Clean context (no research noise)
- Clear requirements (from the spec)
- Focused purpose (just implementation)

All the "should we use library X or Y?" confusion was already resolved. The agent just builds.
...
Your `/implement`, `/test`, `/review`, and `/document` commands can automatically call `/prime` and run `!git diff main` internally. You don't even need to type them.

Your `/implement` command might look like:
...
**Critical warning:** Don't chain these commands together in a single session. Running `/plan` then `/implement` then `/test` all in one session puts you right back into the context pollution problem we're trying to solve.
...
If you're consistently using separate focused sessions, you can **disable auto-compact** for individual sessions.

That's 45,000 tokens back in your pocket.

What can you do with that?
- **Bigger specs.** Handle more complex features.
- **More MCP tools.** Each MCP tool's metadata consumes context, so you can afford a much richer toolset.
- **Deeper problem-solving.** More room for the agent to think.

If you typically use 8-12% of your context for MCP tools, reclaiming 22.5% means you could allocate 18-20% just for tools. A massively more capable agent.
...
With it, you can chain entire workflows:
...
**Note:** You'll need to configure permission hooks to allow automated file edits, which is exactly what hooks are for (see my previous post on taming Claude's YOLO mode).
...
- Base context (`/prime` for project structure and docs)
- Current state (`git diff` for what changed)
...
Nothing new here. We're taking proven software development practices and turning them into reusable, automatable agent workflows.

That's the difference between "vibe coding" (tell the AI what to do and hope) and **agentic engineering** (systematically applying engineering discipline through AI agents).
...
You're not babysitting an unpredictable assistant. You're running a reliable engineering pipeline that happens to be powered by AI.

## Getting started: your first one-session-one-purpose workflow

How to implement this pattern today:
...
Absolutely. The principle is universal: focused sessions with deliberate context handoffs. Implementation details vary by tool, but the pattern works everywhere.
...
Long-running AI coding sessions degrade in performance, eat massive context space, and occasionally go off the rails in catastrophic ways.

The fix isn't giving up on AI agents. It's **structuring how we use them.**
...
This is agentic engineering. Take software development best practices and turn them into reliable, repeatable, automatable AI workflows.
