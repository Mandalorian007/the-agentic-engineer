...
description: "Claude Code is a harness, not a model. Point it at OpenRouter with five environment variables and run Qwen, Kimi, or GLM inside the same agent loop you know."
date: "2026-09-21T11:00:00Z"
category: "tutorials"
hashtags: ["claude-code", "openrouter", "llm", "zsh", "vendor-lock-in"]
social:
  twitter:
    text: "Claude Code never hardcoded api.anthropic.com. Point it at OpenRouter and you pick the model. Better: it runs four model slots, so subagents doing a cheap grep can hit a cheap model while your main loop stays strong."
  linkedin:
    text: "The model and the agent harness are two different products. Most people buy them as one.

The harness is the agent loop, the tool definitions, the context management, the permission system, the subagents. That is the part I actually value in Claude Code, and the part I have invested in: the Marketplace plugin I built at Airbnb combines skills, sub-agents, hooks, and commands on top of it.
...
That is a routing table, not a config chore. Strong model on main for the reasoning, something cheap underneath for the fan-out, each job billed at a rate that matches what it needs. Beats paying one flat rate for everything.
