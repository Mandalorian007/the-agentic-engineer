...
But we're engineers, not prompt magicians. We don't just toss the whole problem at an AI and assume it'll magically work. We build systems.

I recently watched a YouTube video that put words to something I've been seeing firsthand. LLM accuracy falls apart as you scale up iterations. There are engineering patterns that let you chain together massive numbers of AI agent calls reliably, and they're worth talking about.

## The problem: accuracy compounds in sequences

Here's the math that should worry you. If your LLM is 99% accurate, and you ask it to do 100 sequential tasks, you're not getting 99% accuracy overall.
...
The result? LLM accuracy degrades as context increases. That 99% per-task accuracy might drop to 90%, then 80%, and then the agent is hallucinating file paths and making changes to code that doesn't exist.

## Solution 1: state-based isolation

Say you have a single CLI script that needs 100 different changes. That's a lot of work.
...
- History doesn't matter, because the state IS your history

That's the fundamental shift. Use your state as your history. You no longer need to track every intermediate step. The current file state tells you everything you need to know.
...
Say you're using an agent like Claude Code to write code. You send it off with a single task.
...
Say you need to choose an image storage solution for your app. That's a critical decision with long-term implications.
...
Here's the realization that ties everything together. LLMs and agents are API calls.
...
We have enormous power to configure workflows. So why limit yourself to single-shot prompts and hoping for the best?
...
Scenario: you have a CLI script that needs 100 refactoring tasks. Extracting functions, adding type hints, improving error handling, that kind of work.
...
The pattern scales. You could run 1,000 tasks. 10,000 tasks. As long as each individual task maintains high accuracy and you validate at each step, the system stays reliable.
...
The workflows you build, the validation gates, the retry logic, the quality checks, the consensus mechanisms. Those are your actual product. The LLM is just a component.

## Key takeaways

- State isolation beats history accumulation. Use current state as input, skip the context pollution.
- Structure plus validation equals reliability. Structured outputs with automatic retry handle the 1% error rate.
