...
social:
  twitter:
    text: "DORA 2025: AI lifts individual engineers 98% on PRs. Org delivery is flat. That gap is the readout I tell leaders to put on the slide. Not vendor ROI decks, not satisfaction surveys."
  linkedin:
    text: "Engineers ship 98% more PRs with AI. Org delivery is flat. That is the readout, not the vendor ROI deck.\n\nAt Spotify I chaired the Productivity Engineering Technical Steering Group, six teams, and we cut non-business-focused dev cycles by 30%. The discipline that earned that number wasn't a dashboard. It was the willingness to put the inconvenient metric next to the convenient one on the same slide.\n\nThe 2025 DORA report (around 5,000 respondents) is the cleanest public read on the gap right now: 21% more tasks completed, 98% more PRs merged at the individual level, delivery flat at the org level, stability slightly worse. METR's randomized trial backs it up. Experienced developers predicted a 24% AI speedup, self-reported 20%, were measured 19% slower.\n\nIndividual lift is real. Org delivery hasn't translated. Five absorbers eat the gain:\n\n→ Review tax. Authors got faster, reviewers didn't.\n→ PRs got chopped, not work expanded.\n→ Rework. GitClear shows code churn doubled vs. the pre-AI baseline.\n→ Coordination overhead, which AI doesn't touch.\n→ Demand absorbing supply. Good news that reads as bad.\n\nThe slide I push leaders toward is two columns: engineering health (DX Core 4) on the left, AI context on the right. Throughput per team, never per individual. No lines-of-AI-code, no acceptance rate, no token leaderboards. Kent Beck, Charity Majors, and Will Larson all draw the same red line for the same reason.\n\nCurious how other engineering leaders are framing this for their next quarterly. Naming the gap, or still trying to prove the program with vendor ROI numbers?\n\nFull breakdown:"
---
...
If your next quarterly readout needs to hold up to a CFO, [this is the kind of work I help with](/services): vendor-agnostic, outcome-focused, hands-on-keyboard. The measurement layer is never optional, because the alternative is the slide nobody can defend. If that's the readout you're facing, get in touch.
